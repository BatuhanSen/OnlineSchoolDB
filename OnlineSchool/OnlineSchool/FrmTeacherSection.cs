...
{
    public partial class FrmTeacherSection : Form
    {
        public FrmTeacherSection()
        {
            InitializeComponent();
        }
        ...
        void FillGrid()
        {
            try
            {
                da = new SqlDataAdapter("Select * From TEACHER_SECTION", con);
                ds = new DataSet();
                con.Open();
                da.Fill(ds, "TEACHER_SECTION");
                GridList.DataSource = ds.Tables["TEACHER_SECTION"];
                con.Close();
            }
            catch (Exception ex)
            {
        ...
        {
            con.Open();
            cmd = new SqlCommand("SELECT COUNT(*) FROM TEACHER_SECTION WHERE REC_NO ='" + code + "' ", con);
        ...
        private void BtnDelete_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                con.Open();
                cmd.Connection = con;
                cmd.CommandText = "DELETE FROM TEACHER_SECTION WHERE REC_NO = " + REC_NO.Text;
                cmd.ExecuteNonQuery();
                con.Close();
        ...
        {
            if ((TEACHER_ID_NO.Text == "") || (SECTION_NO.Text == ""))
            {
                MessageBox.Show("Check for free fields!", "Info", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            ...
            try
            {
                ...
                con.Open();
                cmd.Connection = con;
                cmd.CommandText = "EXEC PROC_TEACHER_SECTION_INSERT " + REC_NO.Text + "," + TEACHER_ID_NO.Text + ","+ SECTION_NO.Text;
                cmd.ExecuteNonQuery();
                con.Close();
                FieldClear();
                FillGrid();
        ...
        private void GridList_DoubleClick(object sender, EventArgs e)
        {
            ...
            SECTION_NO.Text = GridList.CurrentRow.Cells[2].Value.ToString();
        }

        private void FrmTeacherSection_Load(object sender, EventArgs e)
        {
