...
{
    public partial class FrmStudentLesson : Form
    {
        public FrmStudentLesson()
        {
            InitializeComponent();
        }
        ...
        void FillGrid()
        {
            try
            {
                da = new SqlDataAdapter("Select * From STUDENT_LESSON", con);
                ds = new DataSet();
                con.Open();
                da.Fill(ds, "STUDENT_LESSON");
                GridList.DataSource = ds.Tables["STUDENT_LESSON"];
                con.Close();
            }
            catch (Exception ex)
            {
        ...
        {
            con.Open();
            cmd = new SqlCommand("SELECT COUNT(*) FROM STUDENT_LESSON WHERE REC_NO ='" + code + "' ", con);
        ...
        private void BtnDelete_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                con.Open();
                cmd.Connection = con;
                cmd.CommandText = "DELETE FROM STUDENT_LESSON WHERE REC_NO = " + REC_NO.Text;
                cmd.ExecuteNonQuery();
                con.Close();
        ...
        {
            if ((STUDENT_ID_NO.Text == "") || (LESSON_NO.Text == ""))
            {
                MessageBox.Show("Check for free fields!", "Info", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            ...
            try
            {
                ...
                con.Open();
                cmd.Connection = con;
                cmd.CommandText = "EXEC PROC_STUDENT_LESSON_INSERT " + REC_NO.Text + "," + STUDENT_ID_NO.Text + "," + LESSON_NO.Text;
                cmd.ExecuteNonQuery();
                con.Close();
                FieldClear();
                FillGrid();
        ...
        private void GridList_DoubleClick(object sender, EventArgs e)
        {
            ...
            STUDENT_ID_NO.Text = GridList.CurrentRow.Cells[1].Value.ToString();
            LESSON_NO.Text = GridList.CurrentRow.Cells[2].Value.ToString();
        }

        private void FrmStudentLesson_Load(object sender, EventArgs e)
        {
